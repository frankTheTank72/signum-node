...
public class LegacyDocsServlet extends HttpServlet {
  ...
  private static final Logger logger = LoggerFactory.getLogger(LegacyDocsServlet.class);
  ...
  public LegacyDocsServlet(ApiServlet apiServlet, Set<Subnet> allowedBotHosts, String networkName) {
  ...
  private String buildLinks(HttpServletRequest req) {
    ...
    buf.append("><a href=\"" + API_DOC_PATH + "\">All</a></li>");
    for (LegacyDocTag legacyDocTag : LegacyDocTag.values()) {
      if (requestTags.get(legacyDocTag.name()) != null) {
        buf.append("<li");
        if (requestTag.equals(legacyDocTag.name())) {
          buf.append(" class=\"active\"");
        }
        buf.append("><a href=\"" + API_DOC_PATH + "?requestTag=").append(legacyDocTag.name()).append("\">");
        buf.append(legacyDocTag.getDisplayName()).append("</a></li>").append(" ");
      }
    }
    return buf.toString();
  }
  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse resp) {
    ...
    try {
      try (PrintWriter writer = resp.getWriter()) {
        ...
        writer.print("           <a class=\"navbar-brand\" href=\"" + API_DOC_PATH + "\">" +
  ...
  private String form(String requestType, boolean singleView, String className, List<String> parameters, boolean requirePost) {
    ...
    if (!singleView) {
      buf.append("<a href=\"" + API_DOC_PATH + "?requestType=").append(requestType);
